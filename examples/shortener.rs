...
async fn main() -> Result<()> {
    ...
    let url = "postgres://lawliet:postgres@localhost:5432/shortener";
...
impl AppState {
    ...
    async fn shorten(&self, url: &str) -> Result<String> {
        ...
        let ret: UrlRecord =
            sqlx::query_as("INSERT INTO urls (id, url) VALUES ($1, $2) ON CONFLICT(url) DO UPDATE SET url=EXCLUDED.url RETURNING id")
                .bind(&id)
                .bind(url)
                .fetch_one(&self.db)
                .await?;
